...
(() => {
  ...
  // Alta resolución para que el banner exportado a 1584×396 salga nítido.
  // El simulador exporta leyendo el slot directo (no persiste vía sidecar),
  // así que no importa que el data-URL sea grande.
  const MAX_DIM = 2400;
  ...
  // Host writeFile rejects oversized *.state.json payloads; repeated rejects
  // trip its 'saving paused' circuit-breaker + toast. Skip the write when the
  // snapshot exceeds a safe cap (large/multi photo pages freeze via asset
  // `src` instead, so nothing we rely on is lost).
  const SAVE_CAP_BYTES = 400000;
  function tooBig(str) { return str.length > SAVE_CAP_BYTES; }
  function flushNow() {
    ...
    if (tooBig(payload)) return;
    try { Promise.resolve(w(STATE_FILE, payload)).catch(() => {}); } catch (e) {}
  }
  function save() {
  ...
  // Debounce: coalesce rapid drops into one write so the host save bridge
  // isn't flooded (was tripping 'too many failed saves' on multi-drop pages).
  let saveTimer = null;
  ...
  function setSlot(id, val) {
    ...
    if (loaded) saveDebounced(); else load().then(saveDebounced);
  }
  ...
  const stylesheet =
    ...
    '.ring{position:absolute;inset:0;pointer-events:none;border:var(--slot-ring,1.5px dashed rgba(0,0,0,.25));' +
